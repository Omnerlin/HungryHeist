...
#include <iostream>
    ...
    static std::unordered_map<Collider*, bool> colliders = std::unordered_map<Collider*, bool>();

    void Physics::RegisterCollider(Collider* col) {
        if(colliders.find(col) == colliders.end()) {
            colliders.emplace(col, false);
        } else {
            std::cout << "Tried to register collider " << col << " but it is already registered" << std::endl;
        }
    }
    ...
    void Physics::DeregisterCollider(Collider* col) {
        ...
        if(colItr != colliders.end()) {
            ...
            std::cout << "Tried to deregister nonexistent collider " << col << std::endl;
        }
    }
        ...
        for(auto &col : colliders) {
            for(auto& colTemp : colliders) {
                if(col.first == colTemp.first || colTemp.second == true) {
                    continue; // Don't compare a collider against itself or against colliders that have already run through the list.
                } else {
                    // Check for intersection
                    if(col.first->rect.getGlobalBounds().intersects(colTemp.first->rect.getGlobalBounds())) {
                        if(col.first->isCollidingOrOverlappingWith.find(colTemp.first) == col.first->isCollidingOrOverlappingWith.end()) {
                            // We're colliding or overlapping for the first time
                            col.first->isCollidingOrOverlappingWith.emplace(colTemp.first);
                            colTemp.first->isCollidingOrOverlappingWith.emplace(col.first);
                            col.first->InvokeInitialCallbackAgainstCollider(colTemp.first);
                        }
                        else {
                            // We've collided before and are still colliding
                            //TODO: Add CollisionStay callback? Not needed right now.
                        }
                    }
                    else {
                        // If we were colliding but are now separated
                        auto colItr = col.first->isCollidingOrOverlappingWith.find(colTemp.first);
                        if(colItr != col.first->isCollidingOrOverlappingWith.end()) {
                            // remove the things
